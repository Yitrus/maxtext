...
* ``MegatronMMapDataset.packing_max_segments_per_sample`` default value, preserving the
...
from maxtext.configs.types import MegatronMMapDataset
...
class TestPackingMaxSegmentsPerSampleConfig:
  """``MegatronMMapDataset`` exposes ``packing_max_segments_per_sample`` with the documented default."""
  ...
  def test_zero_disables_merging_round_trip(self):
    ...
    cfg = MegatronMMapDataset(packing_max_segments_per_sample=0, reset_attention_mask=True)
    helper_cfg = _cfg(
