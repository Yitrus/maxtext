...
class TestBlendIndexOutput:
  ...
  def test_size_is_pinned_and_zero_weight_lengths_are_filtered(self):
    ...
    pinned_source = MegatronBlendedDataSource(
        ...
        # Per-dataset ceiling would expand this request to 9 samples.
        size=7,
    )
  ...
  def test_convert_blend_rejects_negative_weights(self, tmp_dir):
    """A negative blend weight must fail before any child conversion starts."""
          ...
          dataset_specs=[
              {
                  "input": [os.path.join(tmp_dir, "does_not_exist")],
                  "weight": -0.1,
                  "output_dir": os.path.join(tmp_dir, "negative"),
              },
              {
                  "input": [os.path.join(tmp_dir, "also_missing")],
                  "weight": 1.1,
                  "output_dir": os.path.join(tmp_dir, "positive"),
              },
          ],
          total_samples=4,
          seq_length=8,
...
class TestCLI:
  ...
  def test_cli_blend_writes_dispatch_pair(self, tmp_dir):
    """The blend subcommand writes child indices and the global dispatch pair."""
    prefix_a = os.path.join(tmp_dir, "source_a")
    prefix_b = os.path.join(tmp_dir, "source_b")
    create_mmap_test_data(prefix_a, [np.arange(64, dtype=np.int32)])
    create_mmap_test_data(prefix_b, [np.arange(100, 164, dtype=np.int32)])
    out_dir = os.path.join(tmp_dir, "output")
    cwd = os.path.join(os.path.dirname(__file__), "..", "..", "src")
    result = subprocess.run(
        [
    ...
    assert result.returncode == 0, f"stderr: {result.stderr}"
    assert os.path.isfile(os.path.join(out_dir, "dataset_index.npy"))
    assert os.path.isfile(os.path.join(out_dir, "dataset_sample_index.npy"))
